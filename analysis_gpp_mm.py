...
gpp_pred = Amax*dfgpp.cond/(K + dfgpp.cond)
#%%
...
def tofit(pars):
    ...
    gpp_pred = Amax*dfgpp.cond/(K + dfgpp.cond)
    resid = (gpp_pred-gpp_arr)
    return resid#[np.isfinite(dfgpp["gpp"])]
#%%
fit0 = np.array([0.5,20,15,200,0.5,0.5,1])
...
for site_id in sym_mean.SITE_ID:
    ...
    def tofit_site(pars):
        ...
        newK = newmax/newslope
        gpp_pred_site = newmax*site_subset.cond/(site_subset.cond + newK)
        return gpp_pred_site-site_subset.gpp
    fit0 = np.array([amp_spring,amp_fall,1])
    ...
    site_subset["gpp_pred2"] = gpp_pred_site
    site_subset["kgpp2"] = newmax/newslope
...
#%%
plt.figure()
plt.plot(newgpp.gpp_pred2,newgpp.gpp,'.',alpha=0.1)
plt.plot([0,15],[0,15])
#%%
newgpp.to_csv("data_gpp_sitewise_nosub_mm2.csv")