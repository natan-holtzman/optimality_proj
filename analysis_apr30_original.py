...
#%%
    ...
    #%%
    dormant = np.zeros(len(z1))
    dormant[dyear.doy < summer_start] = 1
    dormant[dyear.doy > summer_end] = 1
    # yearmax = dyear.groupby("year_new").max(numeric_only=True).reset_index()
    # yearmax["laimax"] = 1*yearmax["LAI"]
    
    # yearmin = dyear.groupby("year_new").min(numeric_only=True).reset_index()
    # yearmin["laimin"] = 1*yearmin["LAI"]
    
    # dy2 = pd.merge(dyear,yearmax[["year_new","laimax"]],on="year_new",how="left")
    # dy2 = pd.merge(dy2,yearmin[["year_new","laimin"]],on="year_new",how="left")

    # #%%
    # dormant = np.array((dy2.LAI - dy2.laimin)/(dy2.laimax - dy2.laimin)) < 0.75
    #%%
    waiting = 0*z1
    z2 = 1*z1 + dormant; z2[-1] = 1
...
#
    gclim = dclim.gpp_nt+dclim.gpp_dt
    dfull["gcmed"] = np.median(dclim.wt[gclim >= 0.5*np.max(gclim)])
    dfull["gcmean"] = np.mean(dclim.wt[gclim >= 0.5*np.max(gclim)])
...
df_meta = df_meta.loc[df_meta.tau_ddreg_hi-df_meta.tau_ddreg_lo < 50].copy()
...
# ax = axes[2,1]
# #ax.scatter(df_meta.summer_end-df_meta.summer_start,df_meta.tau,c=plot_colors)
# ax.scatter(df_meta.fullyear_dmax,df_meta.tau,c=plot_colors)

# ax.set_xlabel("Annual $D_{max}$ (days)")



# ax = axes[2,0]
# #ax.scatter(df_meta.summer_end-df_meta.summer_start,df_meta.tau,c=plot_colors)
# ax.scatter(df_meta.fullyear_dmean,df_meta.tau,c=plot_colors)
# ax.set_xlabel("Annual $D_{mean}$ (days)")
# ax.set_ylabel(r"$\tau$ (days)")
...
plt.plot(tab1.row0,tab1.et_per_F_dm,'o',label=r"US-Me5, $\tau$ = 43 days")
...
for site in pd.unique(ddlist.SITE_ID):
    tabS =  ddlist.loc[ddlist.SITE_ID==site].copy()
    regAll = smf.ols("et_per_F_dm ~ 0 + row0", data=tabS,missing='drop').fit()
    regEach = smf.ols("et_per_F_dm ~ 0 + row0:C(ddi)", data=tabS,missing='drop').fit()
    allpar.append(-regAll.params[0])
    eachpar.append(-np.array(regEach.params))
#%%
# plt.figure()
# for i in range(len(eachpar)):
#     plt.plot([i]*len(eachpar[i]),np.log(eachpar[i]),'bo',alpha=0.5)
# plt.plot(np.log(np.array(allpar)),'ko')
mytab = list(zip(allpar,eachpar))
#%%
mytab = list(sorted(mytab,key = lambda x: x[0]))
#%%
plt.figure()
for i in range(len(mytab)):
    plt.plot([i]*len(mytab[i][1]),np.log(2/mytab[i][1]),'bo',alpha=0.5)
plt.plot(np.log(2/np.array([x[0] for x in mytab])),'ko')