...
df_in["LAIint_rel"] = 1*df_in.LAI
df_in = df_in.loc[np.isfinite(df_in.LAIint_rel)]
df_in = df_in.loc[df_in.vpd > 0.5]

#df_in = df_in.loc[df_in.LAI/df_in.lai_yq95 > 0.9]
...
#%%
    ...
    if len(dfgpp) == 0:
        continue
    #%%
    #dfgpp.dayfrac = 1-(1-dfgpp.dayfrac)*2
    #peakday = dfgpp.doy.loc[dfgpp.LAIint_rel==np.max(dfgpp.LAIint_rel)].iloc[0]
    #%%
    dfgpp.dayfrac = dfgpp.potpar/np.max(dfgpp.potpar)
    #%%
...
#%%
    ...
    #%%
    # plt.figure()
    # losel = dfgpp.airt < np.quantile(dfgpp.airt,0.25)
    # hisel = dfgpp.airt > np.quantile(dfgpp.airt,0.75)
    # plt.plot(dfgpp.normcond[losel],dfgpp.normgpp[losel],'o')
    # plt.plot(dfgpp.normcond[hisel],dfgpp.normgpp[hisel],'o')
    #%%
    ...
    def tofit(pars):
        ...
        t_effect /= np.exp(-(25-pars[2])**2/20**2/2)
        gppmax = t_effect*basemax
        res_cond = max(0,pars[3])
        cond2 = np.clip(Ncond_arr - res_cond,0,np.inf)
        gpp_pred = gppmax*(1-np.exp(-cond2/gppmax*slope))
        return gpp_pred - Ngpp_arr
    fit0 = np.array([np.max(Ngpp_arr),120,25,0.0005])
    cond_optres = scipy.optimize.least_squares(tofit,x0=fit0,method="lm",x_scale=np.abs(fit0))
    pars = cond_optres.x
    ...
    # gpp_pred = gppmax*(1-np.exp(-Ncond_arr/gppmax*slope))
    #%%
    #hicond = dfgpp.loc[normcond > 0.075]
    #lmod = smf.ols("np.log(normgpp) ~ np.log(par) + airt + np.power(airt,2)",data=hicond).fit()
    #lmod2 = smf.ols("np.log(normgpp) ~ np.log(normrad) + airt + np.power(airt,2)",data=hicond).fit()
...
#%%
    #dfmax = fit_gpp_flex_slope_TP_nocond(df_avg)
    #dfmax["gppR2"] = 1- np.mean((dfmax.gpp-dfmax.gpp_pred)**2)/np.var(dfmax.gpp)
...
#%%
rain_data = pd.read_csv("rain_all_mar11.csv")
...
#%%
df_meta = df_meta.loc[df_meta.sgcor < 0.75]
#%%
...
all_results["g_under_75"] = all_results["g_marginal_effect"] < 0.75
all_results["g_over_95"] = all_results["g_marginal_effect"] > 0.95

resmean = all_results.groupby("SITE_ID").mean(numeric_only=True).reset_index()
df_meta = pd.merge(df_meta,resmean[["SITE_ID","g_marginal_effect","g_under_75","g_over_95"]],how='left',on='SITE_ID')
#%%
#df_meta = df_meta.loc[df_meta.g_under_75 > 0.05].copy()
#df_meta = df_meta.loc[df_meta.g_over_95 > 0.05].copy()
