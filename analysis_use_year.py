...
sites_late_season = pd.read_csv("sitewise_gpp_with_year2.csv")
...
sites_late_season["kgpp"] = sites_late_season.gppmax_0 / sites_late_season.slope_0
sites_late_season["gpp_pred"] = 1*sites_late_season["gpp_pred_0"]
...
#%%
    ...
    #%%
#    sgcor = scipy.stats.spearmanr(dfgpp.smc, dfgpp.cond*np.sqrt(dfgpp.vpd/dfgpp.kgpp))
    #sgcor = scipy.stats.spearmanr(dfgpp.smc, dfgpp.cond)
    #vgcor = scipy.stats.spearmanr(dfgpp.vpd, dfgpp.cond)
    #kgcor = scipy.stats.spearmanr(dfgpp.kgpp, dfgpp.cond)


    #dfgpp["sg_rho"] = sgcor[0]
    #dfgpp["sg_pval"] = sgcor[1]

    #dfgpp = dfgpp.loc[dfgpp.par > 500]
    #%%
   
    
    # #dfgpp = dfgpp.loc[dfgpp.waterbal < 0]
    #dfgpp = dfgpp.loc[dfgpp.doy > dfgpp.summer_peak].copy()
    #%%
    # summer_len = (dfgpp.summer_end - dfgpp.summer_start).iloc[0]
    # if len(dfgpp) < summer_len / 2:
    #     continue
    #%%
